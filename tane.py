...
def findCplus(x,dictCplus): # this computes the Cplus of x as an intersection of smaller Cplus sets
	...
	for a in x:
		if x.replace(a,'') in dictCplus.keys():
			...
			temp=findCplus(x.replace(a,''),dictCplus) # compute C+(X\{A}) for each A at a time
...
def compute_dependencies(level, dictCplus,finallistofFDs,listofcolumns,totaltuples,dictpartitions):
	for x in level:
		thesets=[]
		for a in x:
			if x.replace(a,'') in dictCplus.keys():
				...
				temp=computeCplus(x.replace(a,''),listofcolumns,totaltuples,dictpartitions) # compute C+(X\{A}) for each A at a time
				dictCplus[x.replace(a,'')] = temp
			thesets.insert(0, set(temp))
		if list(set.intersection(*thesets)) == []:
			dictCplus[x] = []
	...
	for x in level:
		for a in x:
			if a in dictCplus[x]:
				...
				if validfd(x.replace(a,''), a,totaltuples,dictpartitions): # line 5
...
def computeCplus(x,listofcolumns,totaltuples,dictpartitions): # this computes the Cplus from the first definition in section 3.2.2 of TANE paper. output should be a list of single attributes
	listofcols = listofcolumns[:]
	...
	for a in listofcols:
		for b in x:
			temp = x.replace(a,'')
			temp = temp.replace(b,'')
			if not validfd(temp, b,totaltuples,dictpartitions):
				cplus.append(a)
	return cplus
...
def computeE(x,totaltuples,dictpartitions):
	doublenorm = 0
...
def prune(level,dictCplus,finallistofFDs,dictpartitions):
	...
	for x in level: # line 1
		...
		if check_superkey(x,dictpartitions): # line 4
			temp = dictCplus[x][:]
			...
			for a in temp: # line 5
				...
				for b in x:
					if not( ''.join(sorted((x+a).replace(b,''))) in dictCplus.keys()):
						dictCplus[''.join(sorted((x+a).replace(b,'')))] = findCplus(''.join(sorted((x+a).replace(b,''))),dictCplus)
...
def generate_next_level(level,tableT,dictpartitions):
	nextlevel=[]
	for i in range(0,len(level)): # pick an element
		for j in range(i+1, len(level)): # compare it to every element that comes after it.
			if ((not level[i]==level[j]) and level[i][0:-1]==level[j][0:-1]):  # i.e. line 2 and 3
				...
				if flag==True:
					nextlevel.append(x)
					stripped_product(x, level[i] , level[j],tableT,dictpartitions) # compute partition of x as pi_y * pi_z (where y is level[i] and z is level[j])
	return nextlevel


def stripped_product(x,y,z,tableT,dictpartitions):
	tableS = ['']*len(tableT)
...
def computeSingletonPartitions(listofcols, dictpartitions,data2D):
	for a in listofcols:
		dictpartitions[a]=[]
...
def compute(infile):
	...
	computeSingletonPartitions(listofcolumns, dictpartitions,data2D)
	finallistofFDs=[]
	...
	while (not (L[l] == [])):
		compute_dependencies(L[l],dictCplus,finallistofFDs,listofcolumns,totaltuples,dictpartitions)
		prune(L[l],dictCplus,finallistofFDs,dictpartitions)
		temp = generate_next_level(L[l],tableT,dictpartitions)
		L.append(temp)
		l=l+1
